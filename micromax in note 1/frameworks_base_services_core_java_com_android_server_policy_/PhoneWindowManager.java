...
public class PhoneWindowManager implements WindowManagerPolicy {
    ...
    // By me 
    private void handleExtraButtonPress() {
        final PackageManager pm = mContext.getPackageManager();
        final String selectedPkg = Settings.System.getString(
            mContext.getContentResolver(), "extra_button_app");
            ...
            mHandler.post(new Runnable() {
                ...
                public void run() {
                    ...
                    if (apps == null || apps.size() == 0) {
                        Slog.e(TAG, "No launchable apps found!");
                        return;
                    }
                    final String[] appNames = new String[apps.size()];
                    final String[] pkgNames = new String[apps.size()];
                    for (int i = 0; i < apps.size(); i++) {
                        ResolveInfo ri = apps.get(i);
                        appNames[i] = ri.loadLabel(pm).toString();
                        pkgNames[i] = ri.activityInfo.packageName;
                    }
                    ...
                    builder.setItems(appNames, new DialogInterface.OnClickListener() {
                        ...
                        public void onClick(DialogInterface dialog, int which) {
                            // Save the chosen package name
                            Settings.System.putString(
                                mContext.getContentResolver(), "extra_button_app",
                                pkgNames[which]);
                            // Launch the chosen app immediately
                            Intent launchIntent = pm.getLaunchIntentForPackage(pkgNames[which]);
                    ...
                    // Use a window type allowed for system-level dialogs:
                    dialog.getWindow().setType(WindowManager.LayoutParams.TYPE_PHONE);
                    dialog.show();
                }
            });
        } else{
            // Launch the stored app if it exists
